...
def get_playlist_links(playlist_url):
    ...
    video_elements = [el for el in page_elements if 'pl-video-title-link' in el]
...
parser.add_argument('-p', '--playlisturl', help='url of the playlist to be downloaded', required=True,
                    metavar='')
parser.add_argument('-d', '--destination', help='path of directory to save videos to', default=os.path.curdir,
                    metavar='')
...
if confirmation.lower() in ['y', '']:
    for u in video_urls:
        ...
        if vid.filename in directory_contents:
            print(u'Skipping '+vid.filename)
            continue
        else:
            print(u'Downloading {}'+vid.filename)
